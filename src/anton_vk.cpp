...
VkFramebuffer
createFramebuffer(VkDevice device, VkRenderPass renderPass, VkImageView colorView, VkImageView depthView, u32 width,
                  u32 height) {
...
u32
prepareFrame(Swapchain_t &swapchain, GPUInfo_t &gpu, VkDevice &device, VkSurfaceKHR &surface, VkFormat &swapchainFormat,
             VkCommandBuffer &commandBuffer, VkCommandPool &commandPool,
             VkSemaphore &acquireSemaphore, Buffer_t &uboBuffer, MVPmatrices_t &uboVS,
             Image_t &colorTarget, Image_t &depthTarget, VkFormat &depthFormat,
             VkFramebuffer &targetFramebuffer, VkRenderPass &renderPass,
             VkPipeline &meshPipeline, VkPipelineLayout gfxPipeLayout,
             std::vector<glm::vec4> &pushConstants, f64 elapsedTime, VmaAllocator &vma) {
    SwapchainStatus_t swapchainStatus = updateSwapchain(swapchain, gpu.device, device, surface, swapchainFormat,
                                                        gpu.gfxFamilyIndex);

    auto updateUBO = [&](MVPmatrices_t &uboVS, Buffer_t &uboBuffer, u32 width, u32 height,
    ...
    if (swapchainStatus == Swapchain_Resized || !targetFramebuffer) {
        ...
        createImage(colorTarget, device, swapchain.width, swapchain.height, swapchainFormat,
                    VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, VK_IMAGE_ASPECT_COLOR_BIT,
                    vma);
        createImage(depthTarget, device, swapchain.width, swapchain.height, depthFormat,
                    VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_IMAGE_ASPECT_DEPTH_BIT, vma);
        targetFramebuffer = createFramebuffer(device, renderPass, colorTarget.view, depthTarget.view, swapchain.width,
                                              swapchain.height);
    }


    u32 imageIndex = 0;
    VK_CHECK(vkAcquireNextImageKHR(device, swapchain.swapchain, U64_MAX, acquireSemaphore, /*fence=*/VK_NULL_HANDLE,
                                   &imageIndex));
...
void drawCalls(VkCommandBuffer commandBuffer, Buffer_t &vb, Buffer_t &ib,
               VkPipelineLayout &layout, VkDescriptorSet *descSets, u32 indexCount) {
    ...
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            layout, 0, 1, descSets, 0, nullptr);

    vkCmdDrawIndexed(commandBuffer, indexCount, 1, 0, 0, 0);
}

void submitFrame(u32 imageIndex, VkDevice device, Swapchain_t &swapchain, VkCommandBuffer &commandBuffer, VkQueue queue,
                 Image_t &colorTarget, VkSemaphore &acquireSemaphore, VkSemaphore &releaseSemaphore) {
...
i32 main(i32 argc, const char **argv) {
    ...
    createSwapchain(swapchain, gpu.device, device, surface, swapchainFormat, gpu.gfxFamilyIndex, /*oldSwapchain=*/
                    VK_NULL_HANDLE);
    ...
    while (!glfwWindowShouldClose(windowPtr)) {
        ...
        auto imageIndex = prepareFrame(swapchain, gpu, device, surface, swapchainFormat,
                                       commandBuffer, commandPool,
                                       acquireSemaphore, uboBuffer, uboVS,
                                       colorTarget, depthTarget, depthFormat,
                                       targetFramebuffer, renderPass,
                                       meshPipeline, gfxPipeLayout,
                                       pushConstants, elapsedTime, vma);

        if (imageIndex == U32_MAX) continue;

        drawCalls(commandBuffer, vb, ib, gfxPipeLayout, descSets, cubeMesh.indexCount);

        submitFrame(imageIndex, device, swapchain, commandBuffer, queue,
                    colorTarget, acquireSemaphore, releaseSemaphore);
        ...
        frameCounter++;
        char title[256];
        sprintf(title, "frame: %i - imageIndex: %i - delta time: %f - elapsed time: %f", frameCounter, imageIndex,
